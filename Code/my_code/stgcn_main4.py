...
class Task:
    ...
    def prepare_dataloaders(self, k, k_splits=5, coordinate_dim=3, more_label_1=True):
...
if __name__ == '__main__':
    task = Task()
    task.prepare_dataloaders(k=4, coordinate_dim=3, more_label_1=True)
    task.get_network_architecture(model_name='stgcn_12')
    task.seek_network_weights(model_info='no_pad_loss', serial_no=3, pick_by_f1_score=True)
    task.test(compute_plot=False, save_plot=True)
    task.test_APE()
    task.test_APE_trial()
